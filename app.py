...
def create_game():
    if request.method == 'POST':
        ...
        if game_exists:
            ...
            if user:
                new_game_session = GameSession(title=title, game_id=game_exists.game_id, open_for_join=True, owner=username)
            ...
            if user:
                new_game_session = GameSession(title=title, game_id=new_game.game_id, open_for_join=True, owner=username)
                db.session.add(new_game_session)
                db.session.commit()
